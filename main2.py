...
if req_documento and doc_frente and doc_dorso:
    try:
        ...
        if size_frente > max_doc_bytes:
            app_logger.warning(
                f"[{request_id}] Doc frente demasiado grande: {size_frente} bytes"
            )
            raise HTTPException(
                status_code=413,
        ...
        ext_frente = os.path.splitext(doc_frente.filename)[1] or ".jpg"
        ...
        if size_dorso > max_doc_bytes:
            app_logger.warning(
                f"[{request_id}] Doc dorso demasiado grande: {size_dorso} bytes"
            )
            raise HTTPException(
                status_code=413,
        ...
        ext_dorso = os.path.splitext(doc_dorso.filename)[1] or ".jpg"
        ...
        app_logger.exception(f"[{request_id}] Error guardando documentos")
        raise HTTPException(
            status_code=500,
            detail="Error al guardar las imágenes del documento"
        )
